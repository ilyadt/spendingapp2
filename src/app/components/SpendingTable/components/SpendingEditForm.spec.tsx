...
test('SpendingEditForm', async ({mount, page}) => {
  ...
  await mount(<SpendingEditFormTest budgets={budgets} spending={spending} onSave={onSave} onCancel={onCancel} />)
...
test('SpendingEditForm-cancel-on-overlay', async ({mount, page}) => {
  page.on('dialog', () => null); // prevents auto-dismissing dialog
  ...
  viExpect(onSave).not.toHaveBeenCalled()
  viExpect(onCancel).toHaveBeenCalled()
})
